...
public class AccountControllerTest {
	...
	public void setUp() {
		...
		accountListingRequestDTO = new AccountListingRequestDTO(0,5,"sortBy","sortDirection","searchTerm",searchFields);
	...
	void testAddAccount() throws Exception {
		...
		RequestBuilder request = MockMvcRequestBuilders.post("/api/accounts/add")
				.content(asJsonString( new AccountRequestDTO("accountName",uploadAgreement,false,formData,1,"accountCountry")))
	...
	void testGetAccount() throws Exception{
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/{accountId}", 1))
	...
	void testGetAccountIfDraft() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/draft", 1))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAllAccountsName()throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/names"))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAllAccountsNameAll() throws Exception{
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/names-all"))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAllAccountsFields()throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/fields", 1))
				.andExpect(status().isOk()).andReturn();
	}
	
	@Test
	void testGetAccountListing()throws Exception {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(SerializationFeature.WRAP_ROOT_VALUE, false);
		ObjectWriter writer = mapper.writer().withDefaultPrettyPrinter();
		String requestJson = writer.writeValueAsString(accountListingRequestDTO);
		RequestBuilder request = MockMvcRequestBuilders.post("/api/accounts/listing").content(requestJson)
				.contentType(MediaType.APPLICATION_JSON_VALUE).accept(MediaType.APPLICATION_JSON);
		mockMvc.perform(request).andExpect(status().isOk()).andReturn();
	}
	...
	void testDeleteDraftAccount()throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.delete("/api/accounts/draft/{accountId}",1L))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testSearchAccount() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/search"))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAccountByIdData()  throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts//{accountId}/data",1))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAccountByIdDataAll()throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/{accountId}/data/all",1))
				.andExpect(status().isOk()).andReturn();
	}
	...
	void testGetAllAccountsFieldsAll()throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.get("/api/accounts/fields/all"))
				.andExpect(status().isOk()).andReturn();
	}
}
